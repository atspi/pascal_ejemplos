...
var
	t1,t2,x1,x2,acel,vel0, xprueba, VM, VMMax, SumaVM, PromVM, acelREF, vel0REF : real;
	contVM : Integer;
	isMRUV : Boolean;
	...
	* en función de la ecuación cuadratica, no se puede asumir una dupla inicial (0,0)}
	...
	while t1<>0 do {termino cuando se ingresa un t1=0}
	  begin
			...
			xprueba:=vel0*t2+(acel*t2*t2/2);  {xprueba debe coincidir con x1}
			...
			isMRUV:=(vel0=vel0REF) and (acel=acelREF) and (x2=xprueba); { Verifíco si es MRUV
			( deben tener la misma velocidad inicial y aceleración y x2 debe pertenecer a la ecuación}
		   end;
